...
namespace BarcodeGenerator
{
    public class MainForm : Form
    {
        ...
        private CheckBox chkAppendSuffix; // Nút chọn để bật/tắt hậu tố
        ...
        private ToolTip barcodeToolTip; // ToolTip để hiển thị gợi ý khi di chuột
        ...
        {
            this.Text = "Code Generate"; // Tiêu đề của form
            ...
            // Checkbox để bật/tắt hậu tố
            chkAppendSuffix = new CheckBox
            {
                Text = "Thêm hậu tố\n#1, #2 vào mã",
                Font = new Font("Arial", 9, FontStyle.Regular),
                AutoSize = true,
                Checked = true // Mặc định là bật
            };
            chkAppendSuffix.Location = new Point(rbDataMatrix.Right + radioSpacing, 73);
            ...
            // Khởi tạo ToolTip
            barcodeToolTip = new ToolTip();
            barcodeToolTip.SetToolTip(picBarcode1, "Bấm vào mã code để lưu");
            barcodeToolTip.SetToolTip(picBarcode2, "Bấm vào mã code để lưu");

            // Thêm tất cả điều khiển vào form
            this.Controls.AddRange(new Control[] {
                txtInput, rbQR, rbDataMatrix, chkAppendSuffix,
                btnSave, btnReset,
        ...
        private void UpdateBarcodeLayout()
        {
            ...
            // Di chuyển chkAppendSuffix để nó nằm ngang hàng với radio buttons
            int radioSpacing = 20;
            int totalRadioWidth = rbQR.Width + rbDataMatrix.Width + radioSpacing;
            rbQR.Location = new Point((this.ClientSize.Width - totalRadioWidth) / 2, 70);
            rbDataMatrix.Location = new Point(rbQR.Right + radioSpacing, 70);
            chkAppendSuffix.Location = new Point(rbDataMatrix.Right + radioSpacing + 10, 73); // Điều chỉnh vị trí
        ...
        private void SetupEvents()
        {
            ...
            chkAppendSuffix.CheckedChanged += ChkAppendSuffix_CheckedChanged; // Sự kiện cho checkbox mới
            // Thêm sự kiện Paint cho các PictureBox
            picBarcode1.Paint += PicBarcode_Paint;
            picBarcode2.Paint += PicBarcode_Paint;
            // Thêm sự kiện Click cho các PictureBox để lưu ảnh
            picBarcode1.Click += PicBarcode1_Click;
            picBarcode2.Click += PicBarcode2_Click;
        }

        // Xử lý khi thay đổi trạng thái của CheckBox
        private void ChkAppendSuffix_CheckedChanged(object sender, EventArgs e)
        {
            // Nếu DataMatrix được chọn, luôn ẩn chkAppendSuffix
            if (rbDataMatrix.Checked)
            {
                chkAppendSuffix.Visible = false;
                chkAppendSuffix.Checked = false; // Đảm bảo nó không được chọn khi DataMatrix
            }
            else // Nếu QR Code được chọn
            {
                chkAppendSuffix.Visible = true;
            }
            GenerateBarcode(); // Tạo lại mã vạch khi trạng thái checkbox thay đổi
        }
        ...
        private void BarcodeTypeChanged(object sender, EventArgs e)
        {
            ...
            chkAppendSuffix.Visible = rbQR.Checked; // Ẩn/hiện checkbox khi thay đổi loại mã
        ...
        private void GenerateBarcode()
        {
            ...
            try
            {
                ...
                if (rbQR.Checked)
                {
                    string data1 = baseData;
                    string data2 = baseData;

                    if (chkAppendSuffix.Checked) // Chỉ thêm hậu tố nếu checkbox được chọn
                    {
                        data1 += "#1";
                        data2 += "#2";
                    }
        ...
        // Xử lý khi click vào picBarcode1
        private void PicBarcode1_Click(object sender, EventArgs e)
        {
            if (generatedBarcode1 != null)
            {
                try
                {
                    SaveSingleBarcode("picBarcode1");
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Lỗi khi lưu mã 1: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }

        // Xử lý khi click vào picBarcode2
        private void PicBarcode2_Click(object sender, EventArgs e)
        {
            if (generatedBarcode2 != null && rbQR.Checked) // Chỉ cho phép lưu mã 2 nếu là QR code
            {
                try
                {
                    SaveSingleBarcode("picBarcode2");
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Lỗi khi lưu mã 2: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void SaveQRPair()
        {
            using (FolderBrowserDialog fbd = new FolderBrowserDialog())
            {
                if (fbd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    string path1 = Path.Combine(fbd.SelectedPath, $"{baseName}#1.png"); // Đổi ký tự không hợp lệ trong tên file
                    string path2 = Path.Combine(fbd.SelectedPath, $"{baseName}#2.png"); // Đổi ký tự không hợp lệ trong tên file
        ...
        // Lưu một mã duy nhất (DataMatrix hoặc khi click vào từng mã QR)
        private void SaveSingleBarcode(string source = "")
        {
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                string fileName = MakeValidFileName(txtInput.Text.Trim());
                Bitmap barcodeToSave = null;
                string textToSave = "";

                if (source == "picBarcode1")
                {
                    barcodeToSave = generatedBarcode1;
                    textToSave = lblBarcode1.Text;
                    sfd.FileName = fileName + "#1"; // Tên file cho mã 1
                }
                else if (source == "picBarcode2")
                {
                    barcodeToSave = generatedBarcode2;
                    textToSave = lblBarcode2.Text;
                    sfd.FileName = fileName + "#2"; // Tên file cho mã 2
                }
                else // Từ nút Save chung (chủ yếu cho DataMatrix)
                {
                    barcodeToSave = generatedBarcode1;
                    textToSave = lblBarcode1.Text;
                ...
                if (barcodeToSave != null && sfd.ShowDialog() == DialogResult.OK)
                {
                    SaveWithText(barcodeToSave, textToSave, sfd.FileName);
        ...
        private void BtnReset_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                chkAppendSuffix.Checked = true; // Đặt lại trạng thái mặc định cho checkbox
            }
            catch (Exception ex)
            {
